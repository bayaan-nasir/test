...
from models.covid19.model import build_model, CLASS_NAMES
from training.covid19.dataset import build_dataloaders
...
def evaluate(model, loader, criterion, device):
    ...
    covid_recall = report["COVID-19"]["recall"] if "COVID-19" in report else None
...
def train():
    ...
    logger.info(f"Training COVID-19 classifier on: {device}")
    ...
    best_model_path = settings.model_registry_dir / "covid19" / "best_model.pth"
    ...
    with mlflow.start_run(run_name="covid19_densenet121"):
        ...
        for epoch in range(start_epoch + 1, HPARAMS["num_epochs"] + 1):
            ...
            logger.info(
                ...
                f"covid_recall={val_metrics['covid_recall']}"
            )
            ...
            if val_metrics["covid_recall"] is not None:
                log_dict["val_covid_recall"] = val_metrics["covid_recall"]
        ...
        logger.info(f"COVID-19 recall: {test_metrics['covid_recall']}")
        logger.info(f"Confusion matrix:\n{test_metrics['confusion_matrix']}")

        mlflow.log_metrics({
            "test_acc": test_acc,
            "test_macro_f1": test_metrics["macro_f1"],
            **({"test_covid_recall": test_metrics["covid_recall"]}
               if test_metrics["covid_recall"] is not None else {}),
        })
        logger.info("COVID-19 training complete.")
