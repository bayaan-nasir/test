...
from models.breast_cancer.model import CLASS_TO_IDX
...
class BreastCancerDataset(Dataset):
    """
    Loads breast histology images from BENIGN/ and MALIGNANT/ subfolders.
    Reuses the chest X-ray Albumentations pipeline (224x224, ImageNet norm) —
    works fine for histology since both are single-channel-ish textures
    upsampled to RGB.
    """
    ...
    def _load_samples(self):
        ...
        logger.info(f"BreastCancerDataset [{self.split}] — {len(self.samples)} images loaded")
    ...
    def get_class_weights(self) -> torch.Tensor:
        """
        BreakHis has a roughly 69% malignant / 31% benign split —
        weighted sampling helps prevent the model from over-predicting malignant.
        """
...
def build_dataloaders(
    ...
    datasets = {
        split: BreastCancerDataset(data_dir, split)
