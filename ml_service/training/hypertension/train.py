"""
training/hypertension/train.py
Training pipeline for the Hypertension classifier.

Run from the project root (Windows CMD):
  python -m ml_service.training.hypertension.train
"""
import numpy as np
import mlflow
...
from models.hypertension.model import CLASS_NAMES, FEATURE_NAMES, save_model
from preprocessing.tabular_transforms import load_hypertension_dataframe
from core.config import settings
from core.logger import logger


DATA_PATH = Path("data/raw/hypertension/hypertension.csv")
...
def train():
    if not DATA_PATH.exists():
        logger.error(f"Dataset not found at {DATA_PATH}. Run: python scripts\\download_hypertension_dataset.py")
        return

    df = load_hypertension_dataframe(str(DATA_PATH))
    ...
    with mlflow.start_run(run_name="hypertension_xgb_rf_ensemble"):
        ...
        mlflow.sklearn.log_model(final_pipeline, "hypertension_model")

    save_model(final_pipeline)
    logger.info("Hypertension training complete.")
